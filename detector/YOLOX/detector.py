...
class YOLOx(object):
    ...
    def __call__(self, ori_img):
        ...
        # resize
        ratio = min(self.size[0] / ori_img.shape[0], self.size[1] / ori_img.shape[1])
        ...
        with torch.no_grad():
            ...
            boxes = pred[0]     #　(n,7),  Detections matrix boxes nx7 (xyxy, conf1, conf2, cls), boxes[:, 6]: cls;  boxes[:, 4] * boxes[:, 5]: score
            #　(n,6),  Detections matrix boxes nx6 (xyxy, conf, cls)
            tmp_xyxy_conf =  torch.cat( (boxes[:, 0:4], (boxes[:, 4] * boxes[:, 5]).view(-1,1)), dim=1 ) # (xyxy, conf)
            boxes =  torch.cat( (tmp_xyxy_conf, boxes[:,6].view(-1,1)), dim=1 )
